...
export default {
	...
	methods: {
		...
		callback_getSession: function() {
			...
			this.config = this.$store.state.serverInfo.config; // ローディング直後のコンフィグの初期状態をセット
			...
			this.intervalId = setInterval(function() {
				this.getJson(this.getAPIPath()+'/json/config.json',function(response) { // 生成済みのjsonからコンフィグデータを読み込む
					...
					this.$store.commit('setServerInfo', {config: this.config})
					this.enableList = this.config.statusHome.enableList;
			...
			switch(questionType) {
				...
				case 'questions_4':      // まとめ②
					if( this.enableList[questionType].value == 1 ) {
						if( this.$store.state.session.question_atr[questionType].QUESTION_COMPLETE == true ) {
...
*/
	...
	computed: {
		...mapState({
			imgTopVisual: state => state.serverInfo.imgTopVisual
		})
	},
