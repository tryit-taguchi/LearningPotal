...
/**
 */
class cTransactionAnswer extends cDbaDefault
{
	const TABLE_NAME  = "T_ANSWER";     // 対応するテーブル名
	const PRIMARY_KEY = "ANSWER_ID";    // 対応するプライマリーキー
	...
	static public function getWhere($search = array()) {
		...
		parent::setSimpleCond($where,$cond,$search,"ANSWER_DT");
		parent::setSimpleCond($where,$cond,$search,"MEMBER_ID");
		parent::setSimpleCond($where,$cond,$search,"QUESTION_ID");
	...
	static public function deleteData($search) {
		return parent::deleteData($search);
	}
