...
class cMember
{
	...
	static public function setExcelData($lectureName,$line) {
		$data = array();
		$data['TERM_CD']      = codeCnv($line['A']);            // 利用端末
		$data['SEAT_CD']      = strim($line['B']);              // 席番
	 ...
	 * @return void 無し
	 */
	static public function importData($lectureName,$files) {
	 ...
	 * @return void 無し
	 */
	static public function exportData($lectureName,$siteName) {
		...
		$no = 3;
		foreach($list as $rec) {
			cExcel::cell( 1, $no,$rec['TERM_CD']);                   // 利用端末
			cExcel::cell( 2, $no,$rec['SEAT_CD']);                   // 席番
			$week = $weekList[date("w",strtotime($rec['LECTURE_DT']))];
			//cExcel::cell( 3, $no,str_replace("-","/",$rec['LECTURE_DT'])." ({$week})");                // 日程
	 ...
	 * @return void 無し
	 */
	static public function getData($lectureName,$siteName) {
		$cond = array();
	 ...
	 * @return void 無し
	 */
	static public function saveData($lectureName,$siteName,$lastUpdate) {
		$result = new StdClass();
		$result->status = "success";
		$result->result = new StdClass();
